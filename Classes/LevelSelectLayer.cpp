...
void LevelSelectLayer::loadMenuItem()
{
    ...
    pBtnBack->setPosition(Vec2(150, 200));
    ...
    //                                        });
    // pBtnHelp->setPosition(Vec2(10, 200));
    ...
    _pStartBtn->setPosition(Vec2(150, 100));
    //将三个按钮加入到menu中，不然没有触摸事件
    auto menu = Menu::create(pBtnBack, _pStartBtn, nullptr);
...
void LevelSelectLayer::changeToGameScene()
{
    ...
    if(!std::get<0>(checkLevelIsLockAndBarrierIsClean(_iCurLevelIndex + 1))){
        printf("======levelis lock\n");
    }
